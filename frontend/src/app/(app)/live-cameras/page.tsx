...
import { getCameras, cameraFeedUrl, type ApiCamera } from "@/lib/api";
...
function CameraFeed({ cam }: { cam: ApiCamera }) {
  ...
  const feed = cameraFeedUrl(cam);
  const offline = cam.status !== "online" || broken;

  return (
    <div className="relative aspect-video bg-black overflow-hidden">
      {cam.status === "online" && feed && !broken ? (
        // MJPEG live feed. The AI-annotated primary camera comes straight from
        // the engine's /video; rtsp:// cameras are relayed via /stream.
    ...
    <div className="space-y-6">
      ...
      {fsCam && (
        ...
        >
          ...
          >
            {fsCam.status === "online" && cameraFeedUrl(fsCam) ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img
                src={cameraFeedUrl(fsCam)!}
                alt={fsCam.name}
