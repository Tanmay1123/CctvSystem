        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4" onClick={() => setOpen(false)}>
          <Card className="w-full max-w-lg p-6" >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4" onClick={(e) => e.stopPropagation()}>
              ...
              <Input label="IP Address" value={draft.ipAddress ?? ""} onChange={(v) => setDraft({ ...draft, ipAddress: v })} placeholder="192.168.1.10" />
              <Input
                label="Stream URL"
                value={draft.streamUrl ?? ""}
                onChange={(v) => setDraft({ ...draft, streamUrl: v })}
                placeholder="rtsp://user:pass@192.168.1.10:554/stream"
                hint="Paste the camera's RTSP URL — it's relayed to the browser automatically and appears under Live Cameras. An http MJPEG URL (e.g. the AI feed http://localhost:5000/video) also works and is shown directly."
